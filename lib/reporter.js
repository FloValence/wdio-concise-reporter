...
class ConciseReporter extends events.EventEmitter {
  ...
  prepareConsole(jsonReport) {
    ...
    if (jsonReport.capabilities.version) {
      mainMessage += `:${jsonReport.capabilities.version}`
    }

    if (jsonReport.capabilities.platform) {
      mainMessage += `::${jsonReport.capabilities.platform}`
    }
    ...
    if (jsonReport.capabilities.screenResolution) {
      mainMessage += `::${jsonReport.capabilities.screenResolution}`
    }
    ...
    if (jsonReport.state && jsonReport.state.failed === 0) {
      ...
      jsonReport.fails.forEach(test => {
        ...
        if (test.errorType && test.error) {
          mainMessage += `\t\t ${test.errorType} : \x1b[33m${test.error}\x1b[0m\n`
        }
      })
    }
